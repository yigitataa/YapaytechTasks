...
Marka kataloğundaki `server/data/brands.json` bu kuralın tek istisnasıdır: son başarılı marka
...
Canlı CollectAPI kullanımı için `server/.env.example` dosyasını yerelde `server/.env` olarak
...
Tek komut ortak sözleşmeyi izler ve client/server geliştirme süreçlerini birlikte başlatır.
...
çalıştırıldığını ve terminalde hem `client` hem `server` süreçlerinin başladığını kontrol edin.
Hata ekranındaki `Yeniden dene` düğmesi marka isteğini tekrar gönderir.

Detay ekranında `CollectAPI anahtarı eksik` mesajı görülürse `server/.env.example` dosyasını
`server/.env` olarak kopyalayın, `COLLECTAPI_API_KEY` değerini gerçek anahtarla doldurun ve
...
Başarılı sonuç belleğe ve atomik olarak `server/data/brands.json` dosyasına yazılır.
...
```text
client/src/                      React ekranı, API istemcisi ve UI testleri
server/src/routes/               Express route’ları
server/src/services/             Marka, arama, detay, yakıt ve maliyet adaptörleri
server/src/fixtures/             Açık geliştirme/test modu bağımlılıkları
server/data/brands.json          Son başarılı marka kazıması cache’i
server/test/fixtures/            Ağsız HTML parser fixture’ları
shared/src/                      Zod request/response sözleşmeleri ve ortak tipler
```
