...
describe('extractRawSearchPage', () => {
  ...
  it('tembel yüklenen tüm ilan görsellerinde data-src değerini kullanır', () => {
    const rows = Array.from(
      { length: 6 },
      (_, index) => `
      <tr>
        <td>
          <a href="/ilan/ham-ilan/${index + 1}">
            <img
              src="${index < 3 ? `/images/car-${index + 1}.jpg` : '/ilanfotograflari/noImage/noimage.jpg'}"
              ${index < 3 ? '' : `data-src="/images/car-${index + 1}.jpg"`}
            >
          </a>
        </td>
        <td>Model ${index + 1}</td>
    ...
    const result = extractRawSearchPage(`
      <table>
        <thead><tr><th></th><th>Model</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
    `);

    expect(result.items.map((listing) => listing.imageSrc)).toEqual([
      '/images/car-1.jpg',
      '/images/car-2.jpg',
      '/images/car-3.jpg',
      '/images/car-4.jpg',
